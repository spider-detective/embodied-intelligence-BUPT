...
第2步 登录   ssh a100
第3步 解析   python3 /ssd/data_parse_ros2.py '/ssd/bags/文件名.tar' /ssd/parsed_out/
...
使用convert脚本进行转换，其中转换脚本位于`/ssd/converter/convert_json_2_lerobot_new.py`，环境为conda（wallx）环境中

使用方法如下：

```terminal
python /ssd/converter/convert_json_2_lerobot_new.py   --source-root /ssd/parsed_out_feng   --dataset-root /ssd/lerobot/grab_001   --repo-id local/grab_001   --task "抓取目标物体"
```
